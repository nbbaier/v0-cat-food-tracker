...
import { Edit, HelpCircle, Package, ThumbsDown, ThumbsUp, Trash2 } from "lucide-react"
    ...
    <>
      <Card className="flex flex-col">
        <CardHeader className="flex flex-row justify-between items-center space-y-0">
          <h3 className="text-base font-semibold tracking-tight leading-none sm:text-lg">{food.name}</h3>
        ...
        <CardContent className="flex-1 space-y-3">
          ...
          {food.notes ? (
            <p className="text-sm leading-relaxed text-muted-foreground">{food.notes}</p>
          ) : (
